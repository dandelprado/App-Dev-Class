...
let arrowPressed = { left: false, right: false, up: false, down: false, space: false };
let increment = 0.025;
...
document.addEventListener(
  ...
  function animate() {
    ...
    if (arrowPressed.space) {
      cylinder.position.y += increment;
    } else {
      if(cylinder.position.y > -0.25 && cylinder.position.y != -0.25) {
        cylinder.position.y -= increment;
      } 
    }
