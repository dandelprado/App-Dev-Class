...
setInterval(() => {
  enemies.forEach(enemy => {
    const pos = getValidEnemyPosition();
    if (pos) enemy.position.set(pos.x, 1, pos.z);
  });
}, 3000);
...
function isPositionValid(x, z) {
  return !mazeWalls.some(wall => {
    const dx = Math.abs(wall.position.x - x);
    const dz = Math.abs(wall.position.z - z);
    return dx < 3 && dz < 3;
  }) && !enemies.some(enemy => {
    const dx = Math.abs(enemy.position.x - x);
    const dz = Math.abs(enemy.position.z - z);
    return dx < 3 && dz < 3;
  });
}

function getValidEnemyPosition(maxTries = 100) {
  for (let i = 0; i < maxTries; i++) {
    const x = Math.random() * 90 - 45;
    const z = Math.random() * 90 - 45;
    if (isPositionValid(x, z)) return { x, z };
  }
  return null;
}
